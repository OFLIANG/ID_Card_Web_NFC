...
:root {
    ...
    --accent: #ff6d00;        /* 信号橙 */
    --surface: rgba(10, 18, 40, 0.12);  /* 玻璃表面 */
    --surface-border: rgba(0, 229, 255, 0.10);
    --text: #c8d6e5;          /* 冷灰色 */
    --text-dim: #6b7d94;      /* 暗灰色 */
    --glow: #00e5ff;          /* 发光色 */
}
...
├── main.js         # 交互逻辑（Three.js 地球 + 粒子 + i18n + 震动 + 缩放 + 透明度 + 自适应）
...
| 模块 | 文件位置 | 说明 |
|------|----------|------|
| **Part A** | `main.js` Part A | Three.js 3D 地球场景（地球、卫星、星空、大气辉光） |
...
| `satMaxRadius` | 卫星轨道最大半径（含边距），用于相机自适应 | `2.8` |
...
- **特效**：线框地球（放大版）、星空粒子、流星尾迹、玻璃拟态、扫描线覆盖层、卫星遥感数据条
...
else if (minDim <= 390) targetScale = 0.95;  // 中小屏手机
// 391+ stays at 1.0
```
